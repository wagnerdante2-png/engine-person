export const defaultState = {
  ...
  human: {
    ...
    chestWidth: 1.00, chestDepth: 1.00, glute: 1.00, muscle: 1.00, neckThickness: 1.00,
    legLength: 1.00, armLength: 1.00, bodyMass: 0.95, headScale: 1.00,
    faceWidth: 1.00, headDepth: 1.00, jawWidth: 0.94, cheekbones: 1.00, chinSize: 1.00,
    foreheadHeight: 1.00, browHeight: 1.00, browThickness: 1.00,
  ...
  city: {
    seed: 952731,
    era: 'modern',
    blocksX: 7, blocksZ: 7, density: 0.74, streetWidth: 0.18,
    parcelSubdivision: 0.58, commercialMix: 0.42,
...
function mergeDeep(base, incoming) {
  ...
  const out = { ...base };
  for (const [key, value] of Object.entries(incoming)) out[key] = value && typeof value === 'object' && !Array.isArray(value) ? mergeDeep(base?.[key] ?? {}, value) : value;
  return out;
}
export class Store extends EventTarget {
  constructor(initial = defaultState) { super(); this.state = cloneState(mergeDeep(defaultState, initial)); }
  get(path) { return path.split('.').reduce((acc, key) => acc?.[key], this.state); }
  set(path, value, silent = false) {
    const keys = path.split('.'); let node = this.state;
...
export function mulberry32(seed) {
  let a = seed >>> 0;
  return function rand() { a |= 0; a = (a + 0x6D2B79F5) | 0; let t = Math.imul(a ^ (a >>> 15), 1 | a); t = (t + Math.imul(t ^ (t >>> 7), 61 | t)) ^ t; return ((t ^ (t >>> 14)) >>> 0) / 4294967296; };
}
