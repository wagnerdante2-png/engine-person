...
import { generateCastle } from './generators/castle.js';
...
import { CastlePanelUI } from './ui/castle-panel.js';
...
const castleInspector = new CastlePanelUI(store, regenerate);
...
function renderCurrent() {
  ...
  if (mode === 'human') {
    ...
    const obj = generateCastle(store.state.castle);
    runtime.setObject(obj, { mode:'castle' });
    const s = obj.userData.stats;
    stats.textContent = `Castelo medieval · ${s.towers} torres · ${s.walls} muralhas · fosso ${s.moat?'ativo':'desligado'} · ${s.forestTrees} árvores · ${s.villageHouses} casas externas`;
    modeLabel.textContent = 'MEDIEVAL CASTLE';
    seedLabel.textContent = String(store.get('castle.seed')).padStart(6,'0');
  } else {
...
function renderInspector(mode,tool){ if(mode==='warehouse') warehouseInspector.render(); else if(mode==='supermarket') supermarketInspector.render(); else if(mode==='castle') castleInspector.render(); else inspector.render(mode,tool); }
...
document.querySelector('#randomizeBtn').addEventListener('click',()=>{ const mode = store.get('mode'); if (mode === 'city') store.randomizeSeed('city'); else if(mode === 'warehouse') store.randomizeSeed('warehouse'); else if(mode === 'supermarket') store.randomizeSeed('supermarket'); else if(mode === 'castle') store.randomizeSeed('castle'); else store.randomizeSeed(); renderCurrent(); });
