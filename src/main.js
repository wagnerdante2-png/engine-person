...
function regenerate(path='') {
  if (path.startsWith('environment.')) runtime.applyEnvironment(store.state.environment);
  else renderCurrent();
}
...
function setMode(mode) {
  ...
  inspector.render(mode, store.get('activeTool'));
  renderCurrent();
}

function setTool(tool) {
  store.set('activeTool', tool, true);
  document.querySelectorAll('.tool-button').forEach(btn => btn.classList.toggle('active', btn.dataset.tool === tool));
  inspector.render(store.get('mode'), tool);
}
...
document.querySelector('#randomizeBtn').addEventListener('click',()=>{
  const mode = store.get('mode');
  if (mode === 'city') store.randomizeSeed('city'); else store.randomizeSeed();
  renderCurrent();
});
...
document.querySelector('#saveBtn').addEventListener('click',()=>{
  ...
  a.href=url; a.download=`engine-person-${Date.now()}.json`; a.click();
  URL.revokeObjectURL(url);
});

document.querySelector('#openProject').addEventListener('change', async event => {
  const file = event.target.files?.[0];
  if (!file) return;
  try {
    const data = JSON.parse(await file.text());
    store.replace(data);
    setMode(store.get('mode') ?? 'human');
    setTool(store.get('activeTool') ?? 'shape');
  } catch (err) {
    alert(`Projeto inválido: ${err.message}`);
  } finally {
    event.target.value='';
  }
});
...
document.querySelector('#toggleGridBtn').addEventListener('click',()=>{
  store.set('environment.grid', !store.get('environment.grid'));
  runtime.applyEnvironment(store.state.environment);
});
...
document.querySelector('#collapseInspector').addEventListener('click',()=>{
  const el = document.querySelector('#inspector');
  el.classList.toggle('collapsed');
});

window.addEventListener('keydown',event=>{
  if (event.key.toLowerCase()==='f') runtime.frame(store.get('mode'));
